...
bool addCgroupCpuController(const std::string& cgroupName) {
    std::string controllerPath = "/sys/fs/cgroup/" + cgroupName + "/cgroup.subtree_control";

    std::ofstream controllerFile(controllerPath);
    if (!controllerFile.is_open()) {
        std::perror(("open " + controllerPath).c_str());
        return false;
    }
    
    controllerFile << "+cpu" << std::endl;
    controllerFile.close();
    
    if (controllerFile.fail()) {
        std::perror(("write " + controllerPath).c_str());
        return false;
    }
...
int main() {
    ...
    if (childLow == 0) {
        ...
        if (!addCgroupCpuController("low_weight")) {
            std::cerr << "PID=" << getpid() << ": failed to create low_weight cgroup.\n";
            return 1;
        }

        if (!createCgroup("low_weight/nest")) {
            std::cerr << "PID=" << getpid() << ": failed to create low_weight cgroup.\n";
            return 1;
        }
        if (!setCgroupWeight("low_weight/nest", 500)) {
            std::cerr << "PID=" << getpid() << ": failed to set cgroup weight to 1.\n";
            return 1;
        }

        if (!addProcessToCgroup("low_weight/nest", getpid())) {
            std::cerr << "PID=" << getpid() << ": failed to add process to low_weight/nest cgroup.\n";
            return 1;
        }
        usleep(500);
